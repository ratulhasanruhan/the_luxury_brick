  ...
  return (
    <>
      ...
      <nav className={`nav-bar ${isScrolled ? 'scrolled' : ''}`}>
        ...
        <button className="nav-cta" onClick={() => setCheckoutOpen(true)} id="nav-apply-button">
        ...
        <div className="hero-grid">
          <div className="hero-content reveal">
            ...
            <div className="hero-ctas">
              <a href="#customizer" className="btn-primary" id="hero-bespoke-cta">
                Select Drop Edition
              </a>
              <button className="btn-secondary" onClick={() => setCheckoutOpen(true)} id="hero-apply-cta">
                Request Allocation <ArrowRight size={13} />
              </button>
            </div>
          </div>

          <div className="hero-3d-wrapper reveal" style={{ height: '500px', width: '100%', position: 'relative' }}>
            <BriqueCanvas />
          </div>
        </div>
      ...
      <section id="customizer" className="customizer">
        ...
        <div className="customizer-image-block reveal" style={{
          backgroundImage: 'url(/assets/brique_gallery_2.png)',
          backgroundSize: 'cover',
          backgroundPosition: 'center'
        }}>
          <div className="customizer-image-overlay">
            <div className="label" style={{ marginBottom: '0.5rem' }}>Hand-Embossed Registry</div>
            <h3 style={{ fontSize: '1.5rem', fontFamily: 'var(--serif)', color: 'var(--white)' }}>
              The Serialisation
            </h3>
            <p style={{ fontSize: '0.8rem', color: 'var(--s-500)', marginTop: '0.5rem', lineHeight: '1.6' }}>
              Each drop edition is kiln-embossed with a unique serial from #001 to #999,
              registered permanently in our Swiss physical archive ledger.
            </p>
          </div>
        </div>

        {/* Right: Option panel */}
        <div className="customizer-panel reveal">
          <div>
          ...
          <div className="option-cards">
            ...
            ].map(({ key, name, range, desc, price, gold }) => (
              <div
                key={key}
                className={`option-card ${key} ${finish === key ? 'active' : ''}`}
          ...
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', width: '100%' }}
      ...
      {/* ── Pedigree / Features ── */}
      <section id="pedigree" style={{ background: 'var(--ink-1)', maxWidth: '100%', padding: '7rem 5%' }}>
        <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
          <div className="pedigree-header reveal">
          ...
          <div className="features-grid">
            {[
              ...
              <div key={num} className={`feature-card reveal ${accent ? 'gold-accent' : ''}`}>
      ...
      <section id="gallery" className="gallery">
        ...
        <div className="gallery-carousel reveal" id="gallery-carousel">
          {slides.map((slide, i) => (
            <div
              key={i}
          ...
          <button className="carousel-btn prev" onClick={() => setCurrentSlide(p => (p - 1 + slides.length) % slides.length)} aria-label="Previous">
            <ChevronLeft size={18} />
          </button>
          <button className="carousel-btn next" onClick={() => setCurrentSlide(p => (p + 1) % slides.length)} aria-label="Next">
      ...
      <div className="cta-section">
        <div className="reveal" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <button className="btn-primary" onClick={() => setCheckoutOpen(true)} id="bottom-apply-button">
      ...
      {checkoutOpen && (
        <div className="modal-overlay" onClick={() => setCheckoutOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <button className="modal-close" onClick={() => setCheckoutOpen(false)} aria-label="Close" id="close-modal-btn">
              <X size={16} />
            </button>
            ...
            {!submitted ? (
              ...
              <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                ...
                <button onClick={resetForm} className="btn-primary" style={{ width: '100%' }} id="checkout-ok-btn">
                  Return to Registry
                </button>
              </div>
            )}
